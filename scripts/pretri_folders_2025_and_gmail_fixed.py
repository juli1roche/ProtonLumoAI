...
# IMAP folders to skip (special system folders)
SKIP_FOLDERS = [
...
class PreTriAutomatique:
    ...
    def analyser_dossier(self, dossier_name):
        ...
        try:
            ...
            for mailbox_line in mailbox_list:
                ...
                if self.should_skip_folder(mailbox_name):
                    continue
            ...
            # Try to select folder with error handling
            try:
                status, _ = self.mail.select(f'"{dossier_imap}"')
                if status != "OK":
                    print(f"   ❌ Impossible de sélectionner {dossier_imap}")
                    self.rapport["folders_skipped"].append(dossier_imap)
                    return None
            except imaplib.IMAP4.error as e:
                print(f"   ⚠️  IMAP error selecting {dossier_imap}: {e}")
                self.rapport["folders_skipped"].append(dossier_imap)
                return None
    ...
    def afficher_resume(self):
        ...
        print(f"   Dossiers ignorés: {len(self.rapport['folders_skipped'])}")
        print(f"   Sous-dossiers créés: {len(self.rapport['sous_dossiers_crees'])}")
        print(f"   Emails déplacés: {self.rapport['emails_deplaces']}")
        
        if self.rapport["folders_skipped"]:
            print("\n⚠️  Dossiers ignorés (IMAP spéciaux):")
            for folder in self.rapport["folders_skipped"]:
                print(f"   - {folder}")
    ...
    def run(self):
        """Exécuter le pré-tri"""
        print("\n" + "=" * 60)
        print("🤖 PRÉ-TRI AUTOMATIQUE - Folders/2025 et Gmail (FIXED)")
        print("=" * 60 + "\n")
